...
freqSNP_DHFR <- clusters_DHFR %>%
  ...
  pivot_wider(id_cols = c(source, codon), names_from = "variant", values_from = "freq")
...
freqHap_K13 <- clusters_K13 %>%
  ...
  mutate(
         ...
         variant = case_when(
                             ...
                             str_detect(haplotype, wt_haplotype) ~ "wildtype",
                             TRUE ~ "mutant",
                             ),
         total = sum(count)
         ) %>%
...
freqSNP_MDR1 <- clusters_MDR1 %>%
  ...
  left_join(data.frame(position = positions_MDR1, wildtype = wt_alleles),
            by = c("codon" = "position")) %>%
  mutate(variant = case_when(str_detect(allele, "\\,") ~ "mixed",
                             allele == wildtype ~ "wildtype",
                             allele != wildtype ~ "mutant"),
         freq = paste0(freq, " [", count, "]")
         ) %>%
